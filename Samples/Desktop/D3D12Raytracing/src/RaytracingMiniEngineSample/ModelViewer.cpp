...
class RaytracingMiniEngineSample : public GameCore::IGameApp
{
public:

    RaytracingMiniEngineSample( void ) {}
...
private:
    ...
    // private member for creating particles
...
int wmain(int argc, wchar_t** argv)
{
    ...
    g_DisplayWidth = 1280;
    g_DisplayHeight = 720;
    GameCore::RunApplication(RaytracingMiniEngineSample(), L"RaytracingMiniEngineSample"); 
    return 0;
}

// Shader Values at Startup
...
void RaytracingMiniEngineSample::Startup( void )
{
    ...
    m_RootSig.Finalize(L"RaytracingMiniEngineSample", D3D12_ROOT_SIGNATURE_FLAG_ALLOW_INPUT_ASSEMBLER_INPUT_LAYOUT);
    ...
    // Make a copy of the desc before manipulations
    m_CutoutDepthPSO[0] = m_DepthPSO[0];
    m_ShadowPSO = m_DepthPSO[0];
    // conclude the pipeline state object
...
#define ASSET_DIRECTORY "../../../../../MiniEngine/ModelViewer/"
    ...
    MotionBlur::Enable = true;//false;
    TemporalEffects::EnableTAA = false;//true;
    FXAA::Enable = true; //false
    PostEffects::EnableHDR = true;//false;
    PostEffects::EnableAdaptation = true;//false;
    SSAO::Enable = true;
...
void RaytracingMiniEngineSample::Cleanup( void )
{
    m_Model.Clear();
    Lighting::Shutdown();
}
...
void RaytracingMiniEngineSample::Update( float deltaTime )
{
    ScopedTimer _st(L"Update State");
    ...
    float cosphi = cosf(m_SunInclination * pi * 0.5f);
    float sinphi = sinf(m_SunInclination * pi * 0.5f);
    m_SunDirection = Normalize(Vector3( costheta * cosphi, sinphi, sintheta * cosphi ));
    //m_SunDirection = Vector3(costheta * cosphi, sinphi, sintheta * cosphi);
...
void RaytracingMiniEngineSample::RenderObjects( GraphicsContext& gfxContext, const Matrix4& ViewProjMat, eObjectFilter Filter )
{
    struct VSConstants
    {
    ...
    for (uint32_t i = 0; i < m_Model.m_Header.meshCount; i++)
    {
...
void RaytracingMiniEngineSample::SetCameraToPredefinedPosition(int cameraPosition) 
{
...
void RaytracingMiniEngineSample::RenderLightShadows(GraphicsContext& gfxContext)
{
    using namespace Lighting;

    ScopedTimer _st(L"RenderLightShadows", gfxContext);
...
void RaytracingMiniEngineSample::RenderScene(void)
{
    ...
    {
        ScopedTimer _st(L"Z PrePass", gfxContext);

        gfxContext.SetDynamicConstantBufferView(1, sizeof(psConstants), &psConstants);

        {
            ScopedTimer _st(L"Opaque", gfxContext);
            {
        ...
        {
            ScopedTimer _st(L"Cutout", gfxContext);
            {
    ...
    /*if (!skipDiffusePass)
    {
        ...
        if (!SSAO::DebugDraw)
        {
            ScopedTimer _st(L"Main Render", gfxContext);
            {
    ...
    if (!skipShadowMap)
    {
        if (!SSAO::DebugDraw)
        {
            pfnSetupGraphicsState();
            {
                ScopedTimer _st(L"Render Shadow Map", gfxContext);
    ...
    if (!skipDiffusePass)
    {
        ...
        if (!SSAO::DebugDraw)
        {
            ...
            {
                ScopedTimer _st(L"Render Color", gfxContext);
...
#ifdef _WAVE_OP
                gfxContext.SetPipelineState(EnableWaveOps ? m_ModelWaveOpsPSO : m_ModelPSO[0]);
#else
                {
...
void RaytracingMiniEngineSample::RaytraceShadows(
...
void RaytracingMiniEngineSample::RaytraceDiffuse(
    GraphicsContext& context,
    const Math::Camera& camera,
    ColorBuffer& colorTarget)
{
...
void RaytracingMiniEngineSample::RaytraceReflections(
    GraphicsContext& context,
    const Math::Camera& camera,
    ColorBuffer& colorTarget,
    DepthBuffer& depth, 
    ColorBuffer& normals)
{
    ...
    // Get the View Proj Matrix and mirror the Image for nice Reflections.
    auto m0 = camera.GetViewProjMatrix();
    auto m1 = Transpose(Invert(m0));
    ...
    hitShaderConstants.UseShadowRays = true;
    //hitShaderConstants.UseShadowRays = rayTracingMode == RTM_REFLECTIONS;
    context.WriteBuffer(g_hitConstantBuffer, 0, &hitShaderConstants, sizeof(hitShaderConstants));
    context.WriteBuffer(g_dynamicConstantBuffer, 0, &inputs, sizeof(inputs));
...
void RaytracingMiniEngineSample::RenderUI(class GraphicsContext& gfxContext)
{
...
void RaytracingMiniEngineSample::Raytrace(class GraphicsContext& gfxContext)
{
    ScopedTimer _st(L"Raytrace", gfxContext);
...
void RaytracingMiniEngineSample::CreateParticleEffects()
{
    ...
    Smoke.Size = Vector4(55.0f, 98.0f, 28.0f, 195.0f);
    Smoke.Velocity = Vector4(30.0f, 30.0f, 10.0f, 40.0f);
    Smoke.MassMinMax = XMFLOAT2(1.0, 3.5);
    Smoke.Spread.x = 50.0f;
    Smoke.Spread.y = 60.0f;
